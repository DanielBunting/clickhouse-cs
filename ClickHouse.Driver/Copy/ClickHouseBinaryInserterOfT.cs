...
/// <summary>
/// A stateful, schema-fixed bulk inserter that streams strongly-typed rows into a single ClickHouse INSERT.
/// Typed counterpart of <see cref="ClickHouseBinaryInserter"/>; created via
/// <see cref="ClickHouseClient.CreateBinaryInserter{T}(string, InsertOptions)"/> for a type registered with
/// <see cref="ClickHouseClient.RegisterBinaryInsertType{T}"/>. See <see cref="ClickHouseBinaryInserter"/>
/// for lifetime, threading, and timeout semantics.
/// </summary>
/// <typeparam name="T">The registered POCO type.</typeparam>
public sealed class ClickHouseBinaryInserter<T> : IAsyncDisposable
    where T : class
{
    private readonly ClickHouseClient client;
    private readonly string table;
    private readonly PocoPropertyInfo[] properties;
    private readonly Func<T, object>[] getters;
    private readonly InsertOptions options;

    private ClickHouseClient.InsertPlan plan;
    private PocoBatchSerializer serializer;
    ...
    internal ClickHouseBinaryInserter(ClickHouseClient client, string table, PocoPropertyInfo[] properties, Func<T, object>[] getters, InsertOptions options)
    {
        this.client = client;
        this.table = table;
    ...
    public async Task InitAsync(CancellationToken cancellationToken = default)
    {
        ...
        plan = await client.PrepareInsertAsync(table, properties.Select(p => p.ColumnName), options).ConfigureAwait(false);
        serializer = PocoBatchSerializer.GetByRowBinaryFormat(plan.Options.Format);
        batchSize = plan.Options.BatchSize;
        pending = ArrayPool<T>.Shared.Rent(batchSize);
    ...
    /// <param name="row">The POCO instance to append.</param>
    ...
    /// <param name="rows">The POCO instances to append.</param>
    ...
    private async Task FlushPendingAsync(CancellationToken cancellationToken)
    {
        ...
        serializer.SerializeRows(pending, pendingCount, getters, plan.ColumnTypes, session.Writer);
    ...
    private void ReturnPending()
    {
        if (pending != null)
        {
            ArrayPool<T>.Shared.Return(pending, clearArray: true);
            pending = null;
        }
    }
}
