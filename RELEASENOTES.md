...
* **Streaming bulk inserter** (`ClickHouseClient.CreateBinaryInserter` / `CreateBinaryInserter<T>`): a stateful, schema-fixed inserter — `InitAsync()` resolves the schema once and opens one streaming INSERT, `WriteAsync`/`WriteRowAsync` stream rows into it, `CompleteAsync()` finalizes and returns the count. One inserter == one INSERT request: the per-request cost (connection + query setup) is paid once instead of per batch, client/server overlap, and memory stays bounded to ~one batch. `IAsyncDisposable`; disposing without completing truncates the request so the server rejects the unsent remainder. Bounded by `ClickHouseClientSettings.Timeout`; server `max_execution_time` is set per-insert via `InsertOptions.MaxExecutionTime`. One request is **not** one part nor atomic — ClickHouse re-blocks at `max_insert_block_size`, so large/multi-partition inserts produce multiple independent parts and a mid-stream failure can leave a partial commit.
* **Single streaming binary insert**: new opt-in `InsertOptions.StreamSingleInsert`. When `true`, `InsertBinaryAsync` sends the entire dataset as one streaming HTTP INSERT instead of one request per batch — paying the per-request cost (connection + query setup) once instead of per batch, overlapping client serialization with server ingestion (compressed bytes flush to the server at each `BatchSize` boundary), and keeping memory bounded to ~one batch. This closes most of the throughput gap on large inserts. Requires `MaxDegreeOfParallelism = 1`. One request is **not** atomic — ClickHouse re-blocks at `max_insert_block_size`, so large/multi-partition inserts produce multiple parts that commit independently and a mid-stream failure can leave a partial commit (only a single-block, single-partition insert is atomic). Because it is a single request, `ClickHouseClientSettings.Timeout` (default 2 minutes) must cover the whole insert instead of resetting per batch — raise it for large streaming inserts. Default is `false`; existing behavior is unchanged.
